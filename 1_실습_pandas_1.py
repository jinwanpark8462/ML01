# -*- coding: utf-8 -*-
"""1 실습 Pandas-1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1hwkMPS5MRWF883TMpYHkjTU4Vywyf-h0

winemag-data_first150k.csv  파일을 reviews 로 읽는다.
"""

import pandas as pd

from google.colab import drive
drive.mount('/content/drive')

import os

os.chdir('/content/drive/MyDrive/2021 ai 교육/ML-01-main/day01')

df = pd.read_csv('winemag-data_first150k.csv',index_col = 0)

df

"""리뷰의 디스크립션 컬럼을 desc 로 저장한다.

"""

help(df)

df['description'].sort_values(ascending = False)

df['description']

df

"""first_description 이라는 변수에는, 디스크립션 컬럼의 첫번째 데이터를 저장한다.

"""

first_description = df['description'][0]
first_description





"""first_row 라는 변수에, 첫번째 리뷰 데이터(행)를 저장한다.

"""

first_row = df.loc[0]
first_row



"""리뷰의 description column 의 값들 중, 첫번째부터 10번째 데이터까지를 first_descriptions 변수에 저장한다.

"""

first_description = df['description'][0:10]

first_description



"""리뷰에서 인덱스가 1, 2, 3, 5, 8 인 데이터를,  sample_reviews 변수에 저장한다."""

sample_reviews = df.iloc[[1,2,3,5,8]]

sample_reviews





"""df 라는 변수에, 다음 조건을 만족하는 데이터프레임을 저장하시오. 
인덱스가 0, 1, 10, 100 인 데이터에서, 컬럼이 country, province, region_1, region_2 인 데이터들만 가져와서 저장하시오.
"""

df1 = df.iloc[[0,1,10,100]]

df2 = df1[['country','province','region_1','region_2']]

df2



"""Italy 에서 만들어진 와인에 대해서 italian_wines 이라는 이름으로 데이터프레임을 만드시오. 

"""

Italian_wines = df[df['country']=='Italy']
Italian_wines

df.columns





"""리뷰점수가 95점 이상이고, Australia와 New Zealand 에서 만들어진 와인에 대한 데이터프레임을 top_oceania_wines 이라는 이름의 변수로 저장.

"""

df1 = df[df['points']>=95]
df1

df2 = df1[df1['country']=='Australia'] 
df3 = df1[df1['country'] == 'New Zealand']
df4 = pd.concat([df2,df3])
df4